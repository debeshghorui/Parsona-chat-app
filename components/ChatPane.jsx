...
import { cls, timeAgo } from "../utils/utils"
import { getPersonaDisplayName } from "../utils/personas"

function ThinkingMessage({ personaId, onPause }) {
  const displayName = getPersonaDisplayName(personaId)
  return (
    <Message role="assistant" personaId={personaId}>
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-zinc-500">{displayName} is thinking...</span>
        <button
          onClick={onPause}
      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-4 py-6 sm:px-8">
        ...
        {messages.length === 0 && !isThinking ? (
          ...
          <>
            {messages.map((m) => (
              <div key={m.id} className="space-y-2">
                {editingId === m.id ? (
                  ...
                  <Message role={m.role} personaId={conversation.personaId}>
            ...
            {isThinking && <ThinkingMessage personaId={conversation.personaId} onPause={onPauseThinking} />}
          </>
        )}
      </div>
