...
import { DEFAULT_PERSONA_ID } from "../utils/personas"
  ...
  async function sendMessage(convId, content) {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ query: trimmed, personaId, messages: history }),
      })
      ...
      const reply =
        typeof data.response === "string"
          ? data.response
          : JSON.stringify(data.response ?? "No response from agent.", null, 2)
      ...
      const errorText = err instanceof Error ? err.message : "Failed to reach the agent. Please try again."
    ...
    <div className="h-screen w-full bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
      <div className="md:hidden sticky top-0 z-40 flex items-center gap-2 border-b border-zinc-200/60 bg-white/80 px-3 py-2 backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/70">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <ThemeToggle theme={theme} setTheme={setTheme} ready={themeReady} />
        </div>
      </div>
